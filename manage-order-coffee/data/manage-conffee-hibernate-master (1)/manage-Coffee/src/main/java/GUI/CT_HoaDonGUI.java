...
public class CT_HoaDonGUI extends JFrame {
    ...
    private OrderDetailBLL ctBLL = new OrderDetailBLL();
    private String mahd;
...
//        });
    ...
    public void outModel(DefaultTableModel model, ArrayList<OrderDetail> hd) {     //xuat tu arraylist len table
        Vector data;
        model.setRowCount(0);
        for (OrderDetail h : hd) {
            data = new Vector();
            data.add(h.getId());
            data.add(h.getProduct().getName());
    ...
    {
        if (ctBLL.getCt_hdBLL() == null) {
            ctBLL.listOrderDetail(Integer.parseInt(mahd));
        }
        ArrayList<OrderDetail> cthd = (ArrayList<OrderDetail>) ctBLL.getCt_hdBLL();
        model.setRowCount(0);
        outModel(model, cthd);
    }
